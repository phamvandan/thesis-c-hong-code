...
class SRLandmarkSolver(BaseSolver):
    ...
    def train_step(self, index):
        ...
        loss.backward()
        if index % self.train_opt['batch_accumulate'] == 0:
            self.optimizer.step()
            self.optimizer.zero_grad()
    ...
    def load_model_flex_kv(self, old_model_dict, new_model):
        ...
        for old_k, old_v in old_model_dict.items():
            if old_k in new_state_dict.keys():
                if new_state_dict[old_k].shape == old_v.shape:
                    new_state_dict[old_k] = old_v
                    # print("match", old_k)
                else:
                    print("not match")
        return new_model.load_state_dict(new_state_dict, strict=False)
    ...
    def load(self):
        ...
        def _load_func(m, d):
            if isinstance(m, nn.DataParallel):
                ...
                res = self.load_model_flex_kv(d, m)
            ...
            if self.is_train:
                ...
                if isinstance(raw_checkpoint, dict) and not isinstance(raw_checkpoint, OrderedDict):
                    if 'state_dict' in raw_checkpoint.keys():
                        ...
                        raise KeyError("Model not in checkpoint keys: %s" %
                                       ', '.join(list(checkpoint.keys())))
                else:  # state_dict
    ...
    def get_current_landmark_pair(self):
        ...
        HR = visuals['HR']
        SR = visuals['SR']
        landmark = [np.squeeze(np.array(util.get_peak_points(
            h.cpu().numpy()))) * 4 for h in self.heatmap_list]
    ...
    def save_current_visual(self, img_name):
        ...
        visuals_list = [util.quantize(
            visuals['HR'].squeeze(0), self.opt['rgb_range'])]
        visuals_list.extend([
            util.quantize(s.squeeze(0), self.opt['rgb_range']) for s in visuals['SR']
        ])

        visual_images = torch.stack(visuals_list)
        visual_images = thutil.make_grid(
            visual_images, nrow=len(visuals_list), padding=5)
    ...
    def log_current_visual(self, tb_logger, img_name, current_step):
        ...
        visual_images = torch.stack(visuals_list)
        visual_images = thutil.make_grid(visual_images, nrow=len(
            visuals_list), padding=5, normalize=True, scale_each=True)
        tb_logger.add_image(
        ...
        fig = self.get_current_landmark_pair()
        tb_logger.add_figure(img_name + '_Landmark', fig,
    ...
    def save_current_log(self):
        index = np.arange(start=1, stop=len(
            self.records['lr'])+1, step=1, dtype=int)
