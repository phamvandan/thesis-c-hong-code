...
def merge_heatmap_5(heatmap_in, detach):
    ...
    elif heatmap.size(1) == 19:
        new_heatmap = torch.zeros_like(heatmap[:, :5])
        new_heatmap[:, 0] = heatmap[:, 13] # left eye
        new_heatmap[:, 1] = heatmap[:, 14] # right eye
        new_heatmap[:, 2] = heatmap[:, 9:13].sum(1) # nose
        new_heatmap[:, 3] = heatmap[:, 15:19].sum(1) # mouse
